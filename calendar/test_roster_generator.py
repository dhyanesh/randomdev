...
class TestRosterConstraints(unittest.TestCase):
    ...
    def test_roster_generation(self):
        """Test that a roster is generated."""
        self.assertIsNotNone(self.roster, "Roster generation failed.")

    def test_shift_sizes_and_composition(self):
        """Tests constraints 1, 9, 10, 11."""
        self.assertIsNotNone(self.roster, "Roster not generated, skipping shift size tests.")
        ...
        for day, shifts in self.roster.items():
            ...
            if date.weekday() < 6: # Weekday
                self.assertIn(len(shifts['morning']), [3, 4], f"Day {day}: Morning shift size should be 3 or 4")
                self.assertEqual(len(shifts['afternoon']), 1, f"Day {day}: Afternoon shift size should be 1")
            else: # Sunday
                self.assertIn(len(shifts['morning']), [2, 3], f"Day {day}: Sunday Morning shift size should be 2 or 3")
    ...
    def test_night_shift_quotas(self):
        """Tests constraint 2."""
        self.assertIsNotNone(self.roster, "Roster not generated, skipping night shift quota tests.")
        if not self.roster: return
    ...
    def test_consecutive_leaves(self):
        """Tests constraint 4."""
        self.assertIsNotNone(self.roster, "Roster not generated, skipping consecutive leaves tests.")
        if not self.roster: return
    ...
    def test_post_night_duty(self):
        """Tests constraint 5."""
        self.assertIsNotNone(self.roster, "Roster not generated, skipping post night duty tests.")
        if not self.roster: return
    ...
    def test_hod_schedule(self):
        """Tests constraint 8."""
        self.assertIsNotNone(self.roster, "Roster not generated, skipping HOD schedule tests.")
        if not self.roster: return
    ...
    def test_am_sj_no_afternoon(self):
        """Tests constraint 12."""
        self.assertIsNotNone(self.roster, "Roster not generated, skipping AM/SJ afternoon tests.")
        if not self.roster: return
        
        for day, shifts in self.roster.items():
            self.assertNotIn('AM', shifts['afternoon'], f"AM scheduled for afternoon on day {day}")
            if not (self.year == 2025 and self.month == 10):
